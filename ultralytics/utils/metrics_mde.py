...
This module provides metrics for evaluating depth estimation performance in YOLO models with depth estimation
capabilities.
"""
...
def calculate_depth_accuracy(
    pred_depths: torch.Tensor, gt_depths: torch.Tensor, thresholds: list[float] = [1.25, 1.25**2, 1.25**3]
) -> dict[str, float]:
    """
...
def match_predictions(
    preds: list[torch.Tensor], gt_labels: list[dict], iou_threshold: float = 0.5
...
def evaluate_mde(model, dataloader, device="cuda"):
            ...
            for i, pred in enumerate(preds):
                if pred.boxes is not None and len(pred.boxes) > 0:
                    ...
                    if len(gt_boxes) > 0 and len(gt_depths) > 0:
                        ...
                        matched_pairs = match_predictions([pred_data], [{"boxes": gt_boxes, "depths": gt_depths}])
    ...
    if "mean_depth_error_rate" in metrics:
        print(
            f"Mean Depth Error Rate: {metrics['mean_depth_error_rate']:.2f}% ± {metrics['std_depth_error_rate']:.2f}%"
        )
